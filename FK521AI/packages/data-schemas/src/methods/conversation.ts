...
export interface ConversationMethods {
  getConvoFiles(
    ...
    options?: { user?: string; tenantId?: string },
  ): Promise<string[]>;
...
export function createConversationMethods(
   ...
   */
  async function getConvoFiles(
    conversationId: string,
    options?: { user?: string; tenantId?: string },
  ): Promise<string[]> {
    try {
      const Conversation = mongoose.models.Conversation as Model<IConversation>;
      const filter: FilterQuery<IConversation> = { conversationId };
      if (options?.user) {
      ...
      return (
        ((await Conversation.findOne(filter, 'files').lean()) as IConversation | null)
          ?.files ?? []
      );
    } catch (error) {
