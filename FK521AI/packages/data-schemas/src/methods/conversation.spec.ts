...
import { EModelEndpoint } from 'fk521ai-data-provider';
...
describe('Conversation Operations', () => {
  ...
  describe('getConvoFiles', () => {
    ...
    it('should enforce user and tenant filters when provided', async () => {
      ...
      const matched = await getConvoFiles(conversationId, { user: 'user123', tenantId: 'tenant-a' });
      const denied = await getConvoFiles(conversationId, { user: 'user123', tenantId: 'tenant-b' });

      expect(matched).toEqual(['file-owned']);
