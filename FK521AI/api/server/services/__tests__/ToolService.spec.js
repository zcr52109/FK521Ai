...
const mockReadDifyConsoleConfig = jest.fn();
jest.mock('~/server/utils/difyConsoleConfig', () => ({
  readDifyConsoleConfig: (...args) => mockReadDifyConsoleConfig(...args),
}));
...
describe('ToolService - Action Capability Gating', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockReadDifyConsoleConfig.mockReturnValue({
      sandboxTools: {
  ...
  describe('loadAgentTools (definitionsOnly=true) — action tool filtering', () => {
    ...
    it('should not auto-append workspace tools when generic tools capability is enabled', async () => {
      const capabilities = [AgentCapabilities.tools];
      const req = createMockReq(capabilities);
      mockGetEndpointsConfig.mockResolvedValue(createEndpointsConfig(capabilities));
      ...
      await loadAgentTools({
        req,
        res: {},
        agent: { id: 'agent_123', tools: ['calculator'] },
        definitionsOnly: true,
      });

      const [callArgs] = mockLoadToolDefinitions.mock.calls[0];
      expect(callArgs.tools).toEqual(['calculator']);
      expect(callArgs.tools).not.toContain('workspace_list');
    });

    it('should exclude programmatic tool when bridge config is disabled', async () => {
      const capabilities = [AgentCapabilities.tools, AgentCapabilities.programmatic_tools];
      const req = createMockReq(capabilities);
      mockGetEndpointsConfig.mockResolvedValue(createEndpointsConfig(capabilities));
      mockReadDifyConsoleConfig.mockReturnValue({
        sandboxTools: {
      ...
      await loadAgentTools({
        req,
        res: {},
        agent: { id: 'agent_123', tools: [Constants.PROGRAMMATIC_TOOL_CALLING] },
        definitionsOnly: true,
      });
  ...
  describe('loadToolsForExecution — action tool gating', () => {
    ...
    it('should not create PTC tool when bridge config is disabled', async () => {
      const req = createMockReq([AgentCapabilities.tools, AgentCapabilities.programmatic_tools]);
      req.config = {};
      mockReadDifyConsoleConfig.mockReturnValue({
        sandboxTools: {
          allowProgrammaticToolBridge: false,
