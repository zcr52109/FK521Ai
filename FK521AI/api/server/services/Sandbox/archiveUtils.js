...
const DEFAULT_MAX_ENTRIES = Number(process.env.FK521_PROJECT_ARCHIVE_MAX_ENTRIES || 0);
const DEFAULT_MAX_TOTAL_BYTES = Number(
  process.env.FK521_PROJECT_ARCHIVE_MAX_TOTAL_BYTES || 0,
);
...
const DEFAULT_MAX_EXPANSION_RATIO = Number(process.env.FK521_PROJECT_ARCHIVE_MAX_EXPANSION_RATIO || 200);
...
async function assertArchiveSafetyPolicy({
  inspection,
  ...
  const limitEntries = Number(settings.settings?.limits?.archiveMaxEntries ?? maxEntries);
  const limitTotalBytes = Number(settings.settings?.limits?.archiveMaxTotalBytes ?? maxTotalBytes);
  const memberCount = Number(inspection?.entryCount ?? inspection?.memberCount ?? 0);
  if (limitEntries > 0 && memberCount > limitEntries) {
    throw new Error(`Archive exceeds ${limitEntries} entries`);
  }

  const totalUncompressedBytes = Number(inspection?.totalUncompressedBytes || 0);
  const totalCompressedBytes = Number(inspection?.totalCompressedBytes || 0);
  if (limitTotalBytes > 0 && totalUncompressedBytes > limitTotalBytes) {
    throw new Error(`Archive exceeds ${Math.round(limitTotalBytes / (1024 * 1024))}MB extracted size limit`);
  }
  if (totalCompressedBytes > 0) {
    const ratio = totalUncompressedBytes / totalCompressedBytes;
    if (maxExpansionRatio > 0 && ratio > maxExpansionRatio) {
      throw new Error(`Archive expansion ratio ${ratio.toFixed(2)} exceeds safe limit ${maxExpansionRatio}`);
    }
  }
}
          ...
          fsp
            ...
            .then(() => {
              ...
              readStream.on('data', (chunk) => {
                ...
                if (maxTotalBytes > 0 && extractedBytes > maxTotalBytes) {
...
with tarfile.open(archive_path, 'r:*') as archive:
    for member in archive:
        ...
        if max_total_bytes > 0 and extracted_total > max_total_bytes:
        ...
        with source, open(target, 'wb') as output:
            while True:
                ...
                if not chunk:
                    break
                written += len(chunk)
                if max_total_bytes > 0 and extracted_total - int(getattr(member, 'size', 0) or 0) + written > max_total_bytes:
...
async function extractArchive({
  ...
  const settings = await getSystemSettings();
  const effectiveMaxEntries = Number(settings.settings?.limits?.archiveMaxEntries ?? maxEntries);
  const effectiveMaxTotalBytes = Number(settings.settings?.limits?.archiveMaxTotalBytes ?? maxTotalBytes);
  const archiveFormat = resolveArchiveFormat({ archivePath, format });
  if (!archiveFormat) {
    throw new Error(`Unsupported archive format. Supported formats: ${getSupportedArchiveSummary()}`);
  }
  ...
  const inspection = await inspectArchive({
    ...
    maxEntries: Math.max(effectiveMaxEntries || 0, DEFAULT_INSPECT_MAX_ENTRIES),
  });
  await assertArchiveSafetyPolicy({
    inspection,
