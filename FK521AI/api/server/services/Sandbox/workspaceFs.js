...
const DEFAULT_MAX_FILE_BYTES = Number(process.env.FK521_WORKSPACE_MAX_FILE_BYTES || 10 * 1024 * 1024);
const DEFAULT_MAX_TOTAL_WRITE_BYTES = Number(
  process.env.FK521_WORKSPACE_MAX_TOTAL_WRITE_BYTES || 50 * 1024 * 1024,
);
...
async function enforceWriteQuota({ conversationId, bytes, authContext = {} }) {
  if (bytes > DEFAULT_MAX_FILE_BYTES) {
  ...
  if (total + bytes > DEFAULT_MAX_TOTAL_WRITE_BYTES) {
    throw createWorkspaceError('工作区总写入容量超出限制', 'WORKSPACE_QUOTA_EXCEEDED', 413, {
      maxTotalBytes: DEFAULT_MAX_TOTAL_WRITE_BYTES,
      currentBytes: total,
...
async function workspaceGrepSearch({
  ...
  const files = await collectFilesRecursive(resolved.absolutePath, false, Math.max(maxResults * 20, 2000));
  const flags = caseSensitive ? 'g' : 'gi';
  const searchRegex = regex ? new RegExp(query, flags) : new RegExp(escapeRegexLiteral(String(query)), flags);
  const pageStart = Math.max(0, Number(cursor) || 0);
  const pageLimit = Math.max(1, Math.min(Number(pageSize) || 100, 1000));
  const pageEndExclusive = pageStart + pageLimit;
  ...
  for (const file of files) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (globalMatchIndex >= maxResults) {
        hasMore = true;
        break;
      }
    }
  ...
  return {
    ...
    nextCursor: hasMore ? pageEndExclusive : null,
    hasMore,
    matches,
  };
}
