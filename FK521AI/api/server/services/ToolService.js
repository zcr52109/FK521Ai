...
const { getSandboxSystemPreamble } = require('~/server/services/Sandbox/runtimeContract');
...
const { createLocalSandboxProgrammaticToolCallingTool } = require('~/app/clients/tools/util/localSandboxTools');
const { WORKSPACE_TOOL_NAMES, buildCoreRuntimeInstruction } = require('~/server/services/Platform/runtimeContext');
  ...
  for (let i = 0; i < requiredActions.length; i++) {
    ...
    const handleToolOutput = async (output) => {
      ...
      client.addContentData({
        ...
        index: toolCallIndex,
        type: ContentTypes.TOOL_CALL,
        result: toolCall.function?.output, // Include tool execution result in stream
      });
 ...
 */
/** Native FK521AI tools that are not in the manifest */
const nativeTools = new Set([Tools.execute_code, ...WORKSPACE_TOOL_NAMES]);
 ...
 * @returns {Promise<{
 *   toolDefinitions?: import('@fk521ai/api').LCTool[];
 *   toolRegistry?: Map<string, import('@fk521ai/api').LCTool>;
 *   userMCPAuthMap?: Record<string, Record<string, string>>;
 *   hasDeferredTools?: boolean;
 * }>}
 */
async function loadToolDefinitionsWrapper({ req, res, agent, streamId = null, tool_resources }) {
  ...
  if (
    ...
    (configuredTools[0] === AgentCapabilities.context || configuredTools[0] === AgentCapabilities.ocr)
  ) {
  ...
  const sandboxToolsConfig = readDifyConsoleConfig()?.sandboxTools ?? {};
  const programmaticBridgeEnabled = sandboxToolsConfig.allowProgrammaticToolBridge === true;

  const checkCapability = (capability) => enabledCapabilities.has(capability);
  const areToolsEnabled = checkCapability(AgentCapabilities.tools);
  const actionsEnabled = checkCapability(AgentCapabilities.actions);
  const deferredToolsEnabled = checkCapability(AgentCapabilities.deferred_tools);
  const programmaticToolsEnabled = checkCapability(AgentCapabilities.programmatic_tools);

  const filteredTools = configuredTools.filter((tool) => {
    if (tool === Tools.execute_code) {
      return checkCapability(AgentCapabilities.execute_code);
    }
    if (tool === AgentConstants.PROGRAMMATIC_TOOL_CALLING) {
      return areToolsEnabled && programmaticToolsEnabled && programmaticBridgeEnabled;
    }
  ...
  const uniqueFilteredTools = [...new Set(filteredTools)];

  if (!uniqueFilteredTools || uniqueFilteredTools.length === 0) {
    return { toolDefinitions: [] };
  }

  /** @type {Record<string, Record<string, string>>} */
  let userMCPAuthMap;
  if (configuredTools.some((t) => t.includes(Constants.mcp_delimiter))) {
      ...
      if (!isDomainAllowed) {
        logger.warn(
          `[Actions] Domain "${action.metadata.domain}" not in allowedDomains. ` +
            `Add it to fk521ai.yaml actions.allowedDomains to enable this action.`,
        );
        continue;
      }
  ...
  const hasExecuteCode = uniqueFilteredTools.includes(Tools.execute_code);
  const hasProgrammaticSandbox = uniqueFilteredTools.includes(AgentConstants.PROGRAMMATIC_TOOL_CALLING);

  if (
    hasExecuteCode ||
    hasProgrammaticSandbox
  ) {
    const sandboxPreamble = getSandboxSystemPreamble({ req, agent });
    if (hasExecuteCode) {
      toolContextMap[Tools.execute_code] = sandboxPreamble;
    }
    if (hasProgrammaticSandbox) {
      toolContextMap[AgentConstants.PROGRAMMATIC_TOOL_CALLING] = sandboxPreamble;
    }
  }

  const workspaceContext = buildCoreRuntimeInstruction({
    toolNames: uniqueFilteredTools,
    identityContext: { req, agent },
  });
  if (WORKSPACE_TOOL_NAMES.some((toolName) => uniqueFilteredTools.includes(toolName))) {
    toolContextMap.__workspace_runtime__ = workspaceContext;
  }
...
async function loadAgentTools({
  ...
  const _agentTools = configuredTools.filter((tool) => {
    if (tool === Tools.execute_code) {
      return checkCapability(AgentCapabilities.execute_code);
    } else if (tool === AgentConstants.PROGRAMMATIC_TOOL_CALLING) {
      return areToolsEnabled && programmaticToolsEnabled && programmaticBridgeEnabled;
  ...
  const uniqueAgentTools = [...new Set(_agentTools)];

  if (!uniqueAgentTools || uniqueAgentTools.length === 0) {
    return {};
  }
  ...
  const workspaceContext = buildCoreRuntimeInstruction({
    toolNames: uniqueAgentTools,
    identityContext: { req, agent },
  });
  if (WORKSPACE_TOOL_NAMES.some((toolName) => uniqueAgentTools.includes(toolName))) {
    toolContextMap.__workspace_runtime__ = workspaceContext;
  }
  ...
  const hasActionTools = uniqueAgentTools.some((t) => t.includes(actionDelimiter));
  if (!hasActionTools) {
    return {
      toolRegistry,
      userMCPAuthMap,
  ...
  if (actionSets.length === 0) {
    if (uniqueAgentTools.length > 0 && agentTools.length === 0) {
      logger.warn(`No tools found for the specified tool calls: ${uniqueAgentTools.join(', ')}`);
    }
    return {
      toolRegistry,
      userMCPAuthMap,
  ...
  for (const toolName of uniqueAgentTools) {
    if (ToolMap[toolName]) {
      continue;
    }
...
async function loadToolsForExecution({
  ...
  if (isPTC && toolRegistry && programmaticBridgeEnabled) {
    configurable.toolRegistry = toolRegistry;
    try {
      const ptcTool = createLocalSandboxProgrammaticToolCallingTool({ req });
      allLoadedTools.push(ptcTool);
    } catch (error) {
      logger.error('[loadToolsForExecution] Error creating local PTC tool:', error);
    }
  } else if (isPTC && !programmaticBridgeEnabled) {
    logger.warn(
      `[loadToolsForExecution] Programmatic tool bridge disabled by config. User: ${req.user.id} | Agent: ${agent?.id || 'unknown'}`,
    );
  }
  ...
  if (isPTC && programmaticBridgeEnabled && allLoadedTools.length > 0) {
