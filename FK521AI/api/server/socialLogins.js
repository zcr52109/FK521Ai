...
const { CacheKeys } = require('fk521ai-data-provider');
const { isEnabled, shouldUseSecureCookie } = require('@fk521ai/api');
const { logger, DEFAULT_SESSION_EXPIRY } = require('@fk521ai/data-schemas');
const {
  openIdJwtLogin,
  facebookLogin,
  facebookAdminLogin,
...
function resolveSessionSameSite() {
  const input = String(process.env.SESSION_SAME_SITE || '').trim().toLowerCase();
  if (input === 'none' || input === 'lax' || input === 'strict') {
    return input;
  }
  return 'strict';
}
