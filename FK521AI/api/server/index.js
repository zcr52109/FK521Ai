...
const { getSystemSettings } = require('./services/Config/systemSettings');
...
const PARSER_DEFAULT_LIMIT = process.env.REQUEST_BODY_LIMIT || '50mb';
const ALLOWED_CORS_ORIGINS = String(process.env.CORS_ALLOWED_ORIGINS || '')
  .split(',')
  .map((item) => item.trim())
  .filter(Boolean);
const DEFAULT_SAFE_ORIGINS = [
  'http://localhost:3080',
  'http://127.0.0.1:3080',
  'http://localhost:3000',
  'http://127.0.0.1:3000',
...
function buildCorsOriginValidator() {
  const allowSet = new Set(
    ALLOWED_CORS_ORIGINS.length > 0 ? ALLOWED_CORS_ORIGINS : DEFAULT_SAFE_ORIGINS,
  );
...
function secureHeadersMiddleware(req, res, next) {
  ...
  res.setHeader(
    ...
    `default-src 'self'; img-src 'self' data: blob:; script-src 'self' 'nonce-${cspNonce}'; style-src 'self' 'unsafe-inline'; connect-src 'self'; object-src 'none'; frame-ancestors 'none'; base-uri 'self'; form-action 'self';`,
  );
...
async function csrfProtectionMiddleware(req, res, next) {
  const settings = await getSystemSettings();
  if (settings.settings?.featureToggles?.csrfStrictMode === false) {
    return next();
  }
  ...
  if (
    String(req.path || '').startsWith('/files/upload') ||
    String(req.path || '').startsWith('/models')
  ) {
    return next();
  }
  ...
  if (!csrfSecret || !csrfCookie || !csrfHeader || csrfCookie !== csrfHeader) {
    return res.status(403).json({ message: 'CSRF token validation failed' });
  }
  const expected = crypto.createHmac('sha256', csrfSecret).update('csrf-token').digest('hex');
  if (expected !== csrfCookie) {
    return res.status(403).json({ message: 'CSRF token integrity check failed' });
  }
  return next();
}
...
const startServer = async () => {
  ...
  const jsonBodyLimit = String(runtimeSettings.settings?.limits?.jsonBodyLimit || process.env.JSON_BODY_LIMIT || PARSER_DEFAULT_LIMIT);
  const formBodyLimit = String(runtimeSettings.settings?.limits?.formBodyLimit || process.env.FORM_BODY_LIMIT || PARSER_DEFAULT_LIMIT);
  app.use(express.json(jsonBodyLimit === '0' ? {} : { limit: jsonBodyLimit }));
  app.use(express.urlencoded({ extended: true, ...(formBodyLimit === '0' ? {} : { limit: formBodyLimit }) }));
  ...
  app.use((req, res, next) => {
    if (!req.cookies?.csrf_secret || !req.cookies?.csrf_token) {
      const secret = crypto.randomBytes(32).toString('hex');
      const token = crypto.createHmac('sha256', secret).update('csrf-token').digest('hex');
  ...
  const disableRateLimit = runtimeSettings.settings?.featureToggles?.rateLimitEnabled === false;
  if (!disableRateLimit) {
    app.use(
      '/api',
      rateLimit({
    ...
    app.use(
      ['/api/auth', '/api/files'],
      rateLimit({
        windowMs: Number(process.env.SENSITIVE_RATE_LIMIT_WINDOW_MS || 60_000),
        max: Number(process.env.SENSITIVE_RATE_LIMIT_MAX || 20),
        standardHeaders: true,
        legacyHeaders: false,
        message: { message: 'Too many sensitive endpoint requests.' },
      }),
    );
  }
  ...
  app.use('/fonts', staticCache(appConfig.paths.fonts));
  ...
  app.use('/api/admin', requireAuthenticated, requireRole('admin'), routes.adminAuth);
  app.use('/api/admin/config', requireAuthenticated, requireRole('admin'), routes.adminConfig);
  app.use('/api/admin/grants', requireAuthenticated, requireRole('admin'), routes.adminGrants);
  app.use('/api/admin/groups', requireAuthenticated, requireRole('admin'), routes.adminGroups);
  app.use('/api/admin/roles', requireAuthenticated, requireRole('admin'), routes.adminRoles);
  app.use('/api/admin/users', requireAuthenticated, requireRole('admin'), routes.adminUsers);
  app.use(
    '/api/admin/project-apis',
    requireAuthenticated,
    requireRole('admin'),
    routes.adminProjectApis,
  );
  ...
  app.use('/api/admin/dify-console', requireAuthenticated, requireRole('admin'), routes.adminDifyConsole);
  ...
  app.use((err, req, res, next) => {
    const safeMessage = err?.status && err.status < 500 ? err.message : 'Internal server error';
    logger.error('[server] unhandled middleware error', {
      path: req?.path,
      method: req?.method,
      code: err?.code,
      status: err?.status,
