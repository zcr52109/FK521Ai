...
function loadWorkspaceFs({ sandboxPaths }) {
  ...
  const mockRequire = (request) => {
    ...
    if (request === '~/server/services/Config/systemSettings') {
      return {
...
test('workspace tools: glob/grep pagination/search-replace/delete/todo/summary', async () => {
  ...
  assert.equal(typeof grepPage1.nextCursor, 'string');
  ...
  await assert.rejects(
    workspaceFs.workspaceRead({
      conversationId: 'c1',
      path: '/workspace/workdir/src/a.txt',
      authContext: { user: { id: 'other-user' } },
    }),
    (error) => error.code === 'WORKSPACE_CROSS_TENANT_FORBIDDEN',
  );
});
